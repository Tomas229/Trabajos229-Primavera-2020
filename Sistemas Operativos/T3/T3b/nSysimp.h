...
{                 // Descriptor de una tarea
  ...
  union
  {
    void *msg;
    int rc;
  } send;        // sirve para intercambio de info
  int wake_time; // Tiempo maximo de espera de un nReceive

  void *oxyTask;   //Almacena oxy de un proceso
  void *hydroTask; //Almacena hydro de un proceso
  nH2o h2oFinal;   //Almacena el h2o de un proceso (resultado)
};
...
#define WAIT_OXY STATUS_END + 1         // hizo nCombineOxy y espera a los demás
#define WAIT_HYDRO STATUS_END + 2       // hizo nCombineHydro y espera a los demás
#define WAIT_OXY_TIMEOUT STATUS_END + 3 //hizo nCombineOxy y espera con timeout

// Agregar nuevos estados como STATUS_END+1, STATUS_END+2, ...

#define STATUS_LIST                                                                     \
  {                                                                                     \
    "READY", "ZOMBIE", "WAIT_TASK", "WAIT_REPLY",                                       \
        "WAIT_SEND", "WAIT_SEND_TIMEOUT", "WAIT_READ",                                  \
        "WAIT_WRITE", "WAIT_SEM", "WAIT_MON", "WAIT_COND",                              \
        "WAIT_COND_TIMEOUT", "WAIT_SLEEP", "WAIT_OXY", "WAIT_HYDRO", "WAIT_OXY_TIMEOUT" \
  }
